...
import {ChevronLeft, Image as ImageIcon, Timer, Users, Upload, ScrollText, Sparkles, Check} from "lucide-vue-next"
  ...
  recipe.value.ingredient_groups = data.ingredient_groups.map((group, gIdx) => ({
    ...
    ingredients: group.ingredients.map(ing => {
      ...
      return {
        ...
        translations: [
          { language_code: "fr", data: ing.display_name_fr || "", note: ing.actions?.length ? ing.actions.map(a => a.term_fr).join(", ") : "" },
          { language_code: "en", data: ing.display_name_en || "", note: ing.actions?.length ? ing.actions.map(a => a.term_en).join(", ") : "" }
        ]
      };
    })
  }));
...
<template>
  <div v-if="storedOcr" class="min-h-screen">
      ...
      <div class="max-w-400 mx-auto h-20 px-8 flex items-center justify-between">
        ...
        <div class="flex items-center gap-8">
          <div class="flex items-center gap-3">
            <Button variant="ghost" size="icon" @click="router.back()" class="h-10 w-10 rounded-xl hover:bg-primary/10 hover:text-primary transition-all">
              <ChevronLeft class="w-5 h-5"/>
            </Button>
            <div class="h-8 w-px bg-primary/10 mx-1 hidden md:block"></div>
            <div class="hidden md:block">
              <h2 class="text-lg font-black tracking-tighter leading-none uppercase italic">Studio<span class="text-primary">.</span>ocr</h2>
              <p class="text-[9px] font-bold uppercase tracking-[0.4em] text-muted-foreground/30 mt-1">Verification Engine</p>
            </div>
          </div>
          ...
          <div class="flex bg-black/20 p-1.5 rounded-2xl border border-white/3 shadow-inner">
            <button v-for="mode in ['split', 'editor', 'preview']" :key="mode" @click="viewMode = mode"
              ...
              <div v-if="viewMode === mode" class="w-1.5 h-1.5 rounded-full bg-primary shadow-[0_0_8px_rgba(var(--primary),0.8)]"></div>
              {{ mode }}
            </button>
          </div>
        </div>
        ...
        <div class="flex items-center gap-6">
          <div class="hidden lg:flex items-center gap-1 bg-black/20 p-1 rounded-full border border-white/[0.03]">
          ...
          <Button @click="submit"
            ...
            <div class="absolute inset-0 bg-white/10 translate-y-12 group-hover:translate-y-0 transition-transform duration-500"></div>
    ...
    <main class="max-w-350 mx-auto p-6 md:p-12">
      ...
      <div v-if="viewMode === 'preview'" class="max-w-4xl mx-auto border-none rounded-[3rem] p-4 md:p-12 bg-card shadow-2xl animate-in fade-in zoom-in-95 duration-500">
        <RecipeDisplay :recipe="previewRecipe" :multiplier="1"/>
      </div>

      <div v-else class="flex flex-col lg:flex-row gap-12 md:gap-20 items-start">

        <div :class="[viewMode === 'split' ? 'lg:w-[62%]' : 'w-full']" class="space-y-12 md:space-y-20 w-full animate-in slide-in-from-bottom-4 duration-700">

          <div class="space-y-4">
            <div class="flex items-center gap-2 px-1">
              <Sparkles class="w-4 h-4 text-primary animate-pulse" />
              <label class="text-[10px] font-black uppercase tracking-[0.2em] text-primary/60 italic">AI Suggested Title</label>
            </div>
          ...
          <div class="grid grid-cols-1 md:grid-cols-12 gap-10 p-8 md:p-12 bg-card/40 backdrop-blur-sm border border-white/5 shadow-2xl rounded-[2.5rem]">

            <div class="md:col-span-5 lg:col-span-4 space-y-4">
              <label class="text-[10px] font-black uppercase tracking-widest flex items-center gap-2 text-muted-foreground/60 px-1">
                <ImageIcon class="w-4 h-4"/> Main Visual
              </label>
              <div class="relative group aspect-square bg-muted/20 border-2 border-dashed border-white/10 rounded-[2rem] flex flex-col items-center justify-center overflow-hidden transition-all hover:border-primary/40 hover:bg-primary/5 cursor-pointer">
                <img v-if="mainImagePreview" :src="mainImagePreview" class="object-cover w-full h-full transition-transform duration-700 group-hover:scale-110"/>
            ...
            <div class="md:col-span-7 lg:col-span-8 flex flex-col justify-center gap-8">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-8">
                <div class="space-y-3">
                  <label class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/50 ml-2">Servings</label>
                  <div class="relative group">
                    <Users class="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground/30 group-focus-within:text-primary transition-colors"/>
                    <Input type="number" v-model.number="recipe.servings" class="pl-14 h-16 bg-background/50 rounded-2xl border-none shadow-inner text-xl font-black"/>
                  </div>
                </div>
                <div class="space-y-3">
                  <label class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/50 ml-2">Prep Time</label>
                  <div class="relative group">
                    <Timer class="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground/30 group-focus-within:text-primary transition-colors"/>
                    <Input type="number" v-model.number="recipe.prep_time_minutes" class="pl-14 h-16 bg-background/50 rounded-2xl border-none shadow-inner text-xl font-black"/>
                  </div>
                </div>
                <div class="sm:col-span-2 space-y-3">
                  <label class="text-[10px] font-black uppercase tracking-widest text-muted-foreground/50 ml-2">Cook Time</label>
                  <div class="relative group">
                    <Timer class="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground/30 group-focus-within:text-primary transition-colors"/>
                    <Input type="number" v-model.number="recipe.cook_time_minutes" class="pl-14 h-16 bg-background/50 rounded-2xl border-none shadow-inner text-xl font-black"/>
                  </div>
                </div>
              </div>
            </div>
        ...
        <aside v-if="viewMode === 'split'"
          ...
          <div class="rounded-[2.5rem] border border-white/5 bg-card/30 backdrop-blur-md shadow-2xl flex flex-col h-[calc(100vh-10rem)] overflow-hidden">
            ...
            <div class="p-6 border-b border-white/5 bg-muted/20 flex items-center justify-between shrink-0">
              ...
              <Badge variant="outline" class="text-[9px] font-mono opacity-40 px-3 py-1 rounded-full">SOURCE_TEXT</Badge>
            </div>

            <div class="flex-1 overflow-y-auto p-8 text-[12px] font-mono leading-loose text-muted-foreground/60 whitespace-pre-wrap select-all selection:bg-primary/20 selection:text-primary italic no-scrollbar">
              {{ storedOcr.raw_text }}
            </div>
          </div>
        </aside>
