...
function addExisting(tag: Tag) {
  ...
  if (exists) {
    // FIX: Only remove if it's an 'Existing' type AND matches the ID
    const filtered = props.modelValue.filter(t =>
        !(t.type === 'Existing' && t.id === tag.id)
    )
              ...
              >
                <Check
                    :class="cn('mr-2 h-4 w-4',
                              modelValue.some(t => t.type === 'Existing' && t.id === tag.id)
                                ? 'opacity-100'
                                : 'opacity-0'
                              )"
                />
