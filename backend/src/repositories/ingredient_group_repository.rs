use crate::dto::ingredient_dto::{
    IngredientEditorDto, IngredientRecipeViewDto, IngredientTranslationsDto,
};
use crate::dto::ingredient_group_dto::{
    EditIngredientGroupInput, IngredientGroupEditorDto, IngredientGroupInput,
    IngredientGroupTranslationDto, IngredientGroupViewDto,
};
use crate::errors::Error;
use crate::repositories::{ingredient_repository, unit_repository};
use entity::{
    ingredient_group_translations, ingredient_groups, ingredient_translations, ingredients,
};
...
pub async fn create(
    ...
    for ing_input in ingredient_group_input.ingredients {
        let ing_view =
            ingredient_repository::create_and_link(txn, group.id, ing_input, lang).await?;
...
pub async fn find_by_recipe(
    ...
    for ing in ingredients_list {
        let translation = translations
            ...
            .or_else(|| {
                translations
                    .iter()
                    .filter(|t| t.ingredient_id == ing.id)
                    .find(|t| t.language_code == default_lang)
            });
        ...
        ingredients_map
            .entry(ing.ingredient_group_id)
            .or_default()
            .push(IngredientRecipeViewDto {
                id: ing.id,
                name,
    ...
    let results = groups_with_trans
        .into_iter()
        .map(|(group, trans_list)| {
            let title = trans_list
                .iter()
                .find(|t| t.language_code == lang)
...
pub async fn find_all_by_recipe(
    ...
    for ing in ingredients_list {
        ...
        let translations_for_this_ing = all_translations
            .iter()
            .filter(|t| t.ingredient_id == ing_id);

        let translations_dto: Vec<IngredientTranslationsDto> = translations_for_this_ing
            .clone()
        ...
        .map(|(group, trans_list)| {
            ...
            IngredientGroupEditorDto {
                ...
                translations: trans_list
                    .into_iter()
                    .map(|t| IngredientGroupTranslationDto {
...
pub async fn update(
    ...
    for group_in in ingredient_group {
        ...
        let incoming_g_trans_ids: Vec<Uuid> =
        ...
        let incoming_ing_ids: Vec<Uuid> =
            group_in.ingredients.iter().filter_map(|i| i.id).collect();
