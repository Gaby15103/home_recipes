...
use actix_web::{
    App, HttpRequest, HttpResponse, HttpServer, error, http::header::CONTENT_TYPE,
    middleware::Logger, web, web::Data,
};
use redis::Client;
use std::sync::Arc;
...
fn query_error_handler(
    ...
    let response = match &err {
        error::QueryPayloadError::Deserialize(inner) => {
            HttpResponse::BadRequest().body(format!("Invalid query parameter: {}", inner))
        }
        _ => HttpResponse::BadRequest().body(format!("Bad query string: {}", detail)),
    };
...
pub async fn start(config: Config) -> std::io::Result<()> {
    ...
    let database_pool = new_pool(config.database_url.clone()).expect("Failed to create pool.");
    let database_address =
        SyncArbiter::start(num_cpus::get(), move || DbExecutor(database_pool.clone()));
    let redis_client =
        Arc::new(Client::open(config.redis_url.clone()).expect("Cannot connect to Redis"));
    println!("You can access the server at {}", config.bind_address); // <- print BEFORE running
...
fn static_routes() -> actix_web::Scope {
    web::scope("").service(Files::new("/assets", "./assets").show_files_listing())
}
...
fn recipe_routes() -> actix_web::Scope {
    web::scope("/recipes")
        ...
        .route(
            ...
            web::post().to(recipes::restore_version),
        )
}
/* ----------------------------- Two-Factor ----------------------------- */
...
fn debug_routes() -> actix_web::Scope {
    web::scope("/api/debug").route("", web::post().to(debug::debug_multipart_parsed))
}
